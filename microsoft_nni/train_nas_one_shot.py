...
from nni.nas.nn.pytorch import InputChoice
from nni.nas.nn.pytorch.layers import TransformerEncoderLayer, TransformerDecoderLayer
...
# from nni.nas.evaluator.pytorch.lightning import LightningEvaluator
from nni.nas.evaluator.pytorch.lightning import LightningModule
import nni.nas.strategy as strategy

# ----------------------------------------------------------------------------
# 1) Define a LightningModule that wraps your ModelSpace
# ----------------------------------------------------------------------------
class MyLightningModule(LightningModule):
    ...
    def forward(self, x):
        # so we can do "module(x)" if needed
        return self.model_space(x)
...
# ----------------------------------------------------------------------------
# 2) Build your ModelSpace as normal
# ----------------------------------------------------------------------------
model_space = TransformerModelSpace()

# ----------------------------------------------------------------------------
# 3) Build your LightningModule around the ModelSpace
# ----------------------------------------------------------------------------
lightning_module = MyLightningModule(model_space)

# ----------------------------------------------------------------------------
# 4) Create the LightningEvaluator with your data loaders
# ----------------------------------------------------------------------------
# Suppose you already have train_loader, val_loader as in your code:
...
train_loader = pl.DataLoader(train_dataset, batch_size=64, shuffle=True)
valid_loader = pl.DataLoader(test_dataset, batch_size=64)
...
# ----------------------------------------------------------------------------
# 5) Finally, define a One-Shot strategy and run the experiment
# ----------------------------------------------------------------------------
one_shot_strategy = strategy.RandomOneShot()
exp = NasExperiment(model_space, evaluator, one_shot_strategy)
