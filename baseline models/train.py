...
def main():
    ...
    # Create the dataset.
    # The ChannelSequenceDataset should return, for each user,
    # an input sequence of shape (T-1, 4, 18, 8) and a target of shape (4, 18, 8),
    # where T=3000 (i.e. 2999 time steps as input and the final time step as target).
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (X_batch, Y_batch) in enumerate(dataloader):
            ...
            optimizer.zero_grad()
            pred = model(X_batch)  # Expected output: (batch, 4, 18, 8)
            
            # Split prediction into real and imaginary parts.
