function generate_uma(seed)
    if nargin > 0
        rng(seed, 'twister');
        disp(['UMa: Using random seed: ', num2str(seed)]);
    end
    ...
    configs = {
        ...
        struct('name', 'standard', 'M', 8, 'N', 4, 'pol', 3, 'tilt', 12, 'spacing', 0.5, ...
               'rx_type', 'dipole', 'rx_elements', 2, 'rx_pol', '+/-45°'),
        
        % Config 2: Large array with H/V polarization
        struct('name', 'large_hv', 'M', 10, 'N', 6, 'pol', 5, 'tilt', 10, 'spacing', 0.6, ...
               'rx_type', 'patch', 'rx_elements', 2, 'rx_pol', 'H/V'),
        
        % Config 3: Small array with vertical polarization only
        struct('name', 'small_v', 'M', 6, 'N', 2, 'pol', 1, 'tilt', 15, 'spacing', 0.5, ...
    ...
    for config_idx = 1:length(configs)
        ...
        fprintf('UMa scenario: Processing configuration %s\n', config.name);
